package org.lf.admin.db.pojo;

public class BMZC_ZTStat {
	private Integer appId;
	
	private String deptName;
	
	private Integer wsy;
	
	private Integer syz;
	
	private Integer wx;
	
	private Integer xz;
	
	private Integer sqwx;
	
	private Integer sqbf;
	
	private Integer sqxz;
	
	private Integer xjz;
	
	private Integer bf;
	
	private Integer ydj;
	
	private Integer lyz;
	
	private Integer ghz;
	
	private Integer sjz;

	private Integer wdj;
	
	private Integer xj;	
		
	public Integer getAppId() {
		return appId;
	}

	public void setAppId(Integer appId) {
		this.appId = appId;
	}

	public String getDeptName() {
		return deptName;
	}

	public void setDeptName(String deptName) {
		this.deptName = deptName;
	}

	public Integer getWsy() {
		return wsy;
	}

	public void setWsy(Integer wsy) {
		this.wsy = wsy;
	}

	public Integer getSyz() {
		return syz;
	}

	public void setSyz(Integer syz) {
		this.syz = syz;
	}

	public Integer getWx() {
		return wx;
	}

	public void setWx(Integer wx) {
		this.wx = wx;
	}

	public Integer getXz() {
		return xz;
	}

	public void setXz(Integer xz) {
		this.xz = xz;
	}

	public Integer getSqwx() {
		return sqwx;
	}

	public void setSqwx(Integer sqwx) {
		this.sqwx = sqwx;
	}

	public Integer getSqbf() {
		return sqbf;
	}

	public void setSqbf(Integer sqbf) {
		this.sqbf = sqbf;
	}

	public Integer getSqxz() {
		return sqxz;
	}

	public void setSqxz(Integer sqxz) {
		this.sqxz = sqxz;
	}

	public Integer getXjz() {
		return xjz;
	}

	public void setXjz(Integer xjz) {
		this.xjz = xjz;
	}

	public Integer getBf() {
		return bf;
	}

	public void setBf(Integer bf) {
		this.bf = bf;
	}

	public Integer getYdj() {
		return ydj;
	}

	public void setYdj(Integer ydj) {
		this.ydj = ydj;
	}

	public Integer getLyz() {
		return lyz;
	}

	public void setLyz(Integer lyz) {
		this.lyz = lyz;
	}

	public Integer getGhz() {
		return ghz;
	}

	public void setGhz(Integer ghz) {
		this.ghz = ghz;
	}

	public Integer getSjz() {
		return sjz;
	}

	public void setSjz(Integer sjz) {
		this.sjz = sjz;
	}

	public Integer getWdj() {
		return wdj;
	}

	public void setWdj(Integer wdj) {
		this.wdj = wdj;
	}

	public Integer getXj() {
		return xj;
	}

	public void setXj(Integer xj) {
		this.xj = xj;
	}
	
}
