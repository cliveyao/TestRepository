...
public class WXAppService {
	...
	*/
	...
	private void extractUserList(Tag tag, JSONArray userList, Map<String, Set<Integer>> map) {
		String userid;
	 ...
	 */
	@Transactional(rollbackFor = Exception.class)
	public Map<String, Set<Integer>> syncTag(Integer appId) throws OperException {
		Map<String, Set<Integer>> map = new HashMap<>();
		
		List<Tag> tagList = null;
		JSONArray userList = null;
		JSONArray partyList = null;
		...
		try {
			...
			for (Tag tag : tagList) {
				...
				JSONObject json = TagManager.getUserList(getAccessToken(appId), tag.getTagid());
				
				// 根据json中的userlist标签，依次获取指定标签对应的微信用户详细信息 
				userList = json.getJSONArray(TagManager.USERLIST);
				extractUserList(tag, userList, map);
				
				// 根据json中的partylist标签，依次获取指定标签对应的微信用户详细信息 
				partyList = json.getJSONArray(TagManager.PARTYLIST);
				int id;
				for (int i = 0; i < partyList.size(); i++) {
					id = partyList.getIntValue(i);
					userList = DepartmentManager.simpleUserList(getAccessToken(appId), id, true).getJSONArray(TagManager.USERLIST);
					extractUserList(tag, userList, map);
				}
			}
		} catch (WXException e) {
	 ...
	 * 此操作非常耗费资源。请用户谨慎使用。
	 * 
	 * 新的同步过程中，我们将仅同步chu_app表中dept_root指向的部门（及其子部门）下的用户。
	 * 
	 * @throws OperException 无法获取微信组织架构, 微信应用已停用
	 */
	@Transactional(rollbackFor = Exception.class)
	public void syncDepartmentsAndWXUsers(Integer appId, Map<String, Set<Integer>> tagMap) throws OperException {
		List<Department> deptList = null;
		ChuWXUser wxUser = null;
		Map<String, ChuWXUser> wxUserMap = new HashMap<>();
		try {
			int deptRoot = appDao.selectByPrimaryKey(appId).getDeptRoot();
			
			// 获取所有部门信息
			deptList = DepartmentManager.getDepartmentList(getAccessToken(appId), deptRoot);
			...
			for (Department dept : deptList) {
				...
				for (User user : userList) {
					...
					Set<Integer> tagNoList = tagMap.get(user.getUserid());
	...
	private Map<String, Set<Integer>> getTagMap(Integer appId) throws OperException {
		Map<String, Set<Integer>> map = new HashMap<>();
	...
	/**
	 * 这里我们根据appId获得该应用的deptRoot，然后列出该root下的部门，及其子部门编号。
	 * 依据这些编号，对department字符串进行过滤，仅保留root及其子部门信息。
	 */
	private String filterDepartment(Integer appId, String department) {
		if (StringUtils.isEmpty(department)) {
			return department;
		}
		
		int deptRoot = appDao.selectByPrimaryKey(appId).getDeptRoot();
		List<String> deptList = deptService.getSubDeptmentByDeptNo(appId, deptRoot);
		
		JSONArray deptArray = JSONArray.parseArray(department);
		JSONArray filterDeptArray = new JSONArray();
		for (int i = 0; i < deptArray.size(); i++) {
			if (deptList.contains(deptArray.getString(i))) {
				filterDeptArray.add(deptArray.getIntValue(i));
			}
		}
		
		return filterDeptArray.toJSONString();
	}
	 ...
	 * @return
	 */
	public void syncChuWXUser(Integer appId, String... useridList) throws OperException {
		Map<String, Set<Integer>> tagMap = getTagMap(appId);
		for (String userid : useridList) {
			boolean insert = false;
			try {
				// 获取微信服务器上的user数据
				User user = UserManager.getUser(getAccessToken(appId), userid);
				if (user == null) {
					throw new OperException(WXUserService.微信用户不存在);
				}
				...
				if (wxUser == null) {
					// 如果本地数据库不存在，则在chu_wxuser中插入一条记录
					insert = true;
					wxUser = new ChuWXUser();
					wxUser.setAppId(appId);
				...
				wxUser.setName(user.getName());
				wxUser.setDepartment(filterDepartment(appId, user.getDepartment()));
				wxUser.setPosition(user.getPosition());
				wxUser.setMobile(user.getMobile());
				if (user.getGender() != null) {
					wxUser.setGender(Integer.parseInt(user.getGender().getValue()));
				}
				wxUser.setEmail(user.getEmail());
				wxUser.setAvatar(user.getAvatar());
				wxUser.setExtattr(user.getExtattr());
				if (user.getStatus() != null) {
					wxUser.setStatus(Integer.parseInt(user.getStatus().getValue()));
				}
