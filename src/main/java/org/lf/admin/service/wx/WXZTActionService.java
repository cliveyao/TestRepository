...
public class WXZTActionService {
	private static final Logger logger = LoggerFactory.getLogger(WXZTActionService.class);
	@Autowired
	...
	public AjaxResultModel getFJPicker(Integer appId) {
		...
		try {
			...
			logger.error("返回前台poppicker需要的房间三级联动数据出错", e);
			result.setCode(WXResultCode.ERROR.getCode());
			result.setMsg(e.getMessage());
		}
		return result;
	...
	public AjaxResultModel getCFDDByFjid(Integer fjid) {
		...
		try {
			...
			logger.error("根据房间id返回存放地点详细信息出错", e);
			result.setCode(WXResultCode.ERROR.getCode());
			result.setMsg(e.getMessage());
		}
		return result;
	...
	public AjaxResultModel getCFDDByZcid(Integer zcid) {
		...
		try {
			...
			logger.error("根据资产id返回存放地点详细信息出错", e);
			result.setCode(WXResultCode.ERROR.getCode());
			result.setMsg(e.getMessage());
		}
