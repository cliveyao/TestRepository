...
public class RWGLService {
	...
	public JRW createRCXJ(Integer appId, Date kssj, Date jssj, String czr, String czRemark, String ysr, 
		...
		for (Integer zcid : zcidList) {			
			...
			remark.append(userService.getWXUser(appId, cjr));
			remark.append("于").append(DateUtils.getLongDate(new Date()));
			remark.append("创建了一个日常巡检任务");
...
//		获取当前任务下所有资产信息
		...
		for (JZC zc : zcList) {
			...
			StringBuilder remark = new StringBuilder();
			remark.append(userService.getWXUser(zc.getAppId(), cjr));
			remark.append("于").append(DateUtils.getLongDate(new Date()));
			remark.append("删除了一个尚未开始的日常巡检任务");
			zt.setRemark(remark.toString());
			ztService.insertZT(zt);
		}
...
//		获取当前任务下所有资产信息
		...
		for (JZC zc : zcList) {
			...
			remark.append(userService.getWXUser(zc.getAppId(), cjr));
			remark.append("于").append(DateUtils.getLongDate(new Date()));
			remark.append("删除了一个尚未开始的故障维修任务");
			zt.setRemark(remark.toString());
			ztService.insertZT(zt);
		}
