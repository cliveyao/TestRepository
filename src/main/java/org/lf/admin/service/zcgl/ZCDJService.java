...
public class ZCDJService {
	...
	public JZC registZC(Integer appId, String zcdm, String zcmc, Integer zcLx, BigDecimal cost, Integer num,
		...
		Integer yhpCount = yhpService.countYHPList(appId, null, null);
		if (zcCount + yhpCount + 1 > maxCount) {
			throw new OperException(YHPService.资产总数超出预设配额);
		}
		...
		resultSb.append("由").append(userService.getWXUser(appId, djr).getName()).append("登记入库");
	...
	/**
	 * 部门资产管理员：资产登记
	 * 对新购资产进行登记入库。与后勤管理员进行资产登记不同，这里资产将自动分配给资产管理员所在部门，并设置存放地点。
	 * 资产登记成功后，资产状态置为“未使用”资产。
	 * 
	 * 1. 创建一个资产，将资产状态设为“未使用”
	 * 2. 将资产插入到J_ZC表中。
	 * 3. 在L_ZT表中插入一条记录。
	 * 		记录人：djr
	 * 	原状态：空
	 ...
	 */
	@Transactional(rollbackFor = Exception.class)
	public JZC registBMZC(Integer appId, String zcdm, String zcmc, Integer zcLx, String cfdd, BigDecimal cost, Integer num,
			              String xh, String ccbh, Date gzsj, BigDecimal zjnx, String djr,String picUrl) throws OperException {
		...
		zc.setPicUrl(picUrl);
		// 查看当前登记人是否为部门资产管理人
		CZCGL zcgl = zcglService.getGLR(appId, djr);
		zc.setZcglId(zcgl.getId());
		jzcMapper.insertSelective(zc);
	 ...
	 */
	@Transactional(rollbackFor = Exception.class)
	private JZC unregistZC(Integer appId, String zcdm, String zcmc, Integer zcLx, 
		...
		resultSb.append(zcmc).append("（").append(zcdm).append("）");
		resultSb.append("于").append(DateUtils.getLongDate(new Date()));
		resultSb.append("由").append(userService.getWXUser(appId, djr).getName()).append("负责批量入库");
		zt.setRemark(resultSb.toString());
		ztService.insertZT(zt);
	 ...
	 */
	@Transactional(rollbackFor = Exception.class)
	private JZC unregistBMZC(Integer appId, String zcdm, String zcmc, Integer zcLx, 
			              String xh, String ccbh, Date gzsj, BigDecimal zjnx,Integer zcglId, String djr,BigDecimal cost,Integer num) throws OperException {
	 ...
	 */
	@Transactional(rollbackFor = Exception.class)
	public void insertBMZCListByExcel (Integer appId,MultipartFile file,String djr) throws OperException, IOException {
		...
		Integer zcCount = zcService.countZCList(param);
		Integer yhpCount = yhpService.countYHPList(appId, null, null);
		if (zcCount + yhpCount + zcList.size() > maxCount) {
			throw new OperException(YHPService.资产总数超出预设配额);
		}
		//找到djr所在的部门(c_zcgl的id)
		CZCGL zcgl = new CZCGL();
		zcgl.setAppId(appId);
		zcgl.setGlr(djr);
		zcgl = zcglService.getZCGL(zcgl);
		for (JZC zc : zcList) {
			unregistBMZC(appId, zc.getDm(), zc.getMc(), zc.getLxId(), zc.getXh(), zc.getCcbh(), zc.getGzsj(),zc.getZjnx(),zcgl.getId(), djr,zc.getCost(),zc.getNum());
		}
	}
	/**
	 * 根据导入的excel表格导入数据
	...
	public void insertZCListByExcel (Integer appId,MultipartFile file,String djr) throws OperException, IOException {
		...
		for (JZC zc : zcList) {
			unregistBMZC(appId, zc.getDm(), zc.getMc(), zc.getLxId(), zc.getXh(), zc.getCcbh(), zc.getGzsj(),zc.getZjnx(),null, djr,zc.getCost(),zc.getNum());
		}
	}
	/**
	...
	public void checkUnregistedZC(String djr,Integer zcId, String zcmc, Integer zcLx, BigDecimal cost,
		...
		resultSb.append("于").append(DateUtils.getLongDate(new Date()));
		resultSb.append("由").append(userService.getWXUser(resultZc.getAppId(), djr).getName()).append("进行资产登记审核");
		zt.setRemark(resultSb.toString());
	...
	public void delRegistedZC(Integer zcId, String djr) throws OperException {
		...
		resultSb.append(resultZc.getMc()).append("（").append(resultZc.getDm()).append("）");
		resultSb.append("于").append(DateUtils.getLongDate(new Date()));
		resultSb.append("由").append(userService.getWXUser(resultZc.getAppId(), djr).getName()).append("将它从已登记资产库中删除");
		zt.setRemark(resultSb.toString());
		lztMapper.insertSelective(zt);
	}
	...
	public void allocateZC(Integer zcId, Integer zcglId, String cfdd, String djr) throws OperException {
		...
		StringBuilder resultSb = new StringBuilder();
		resultSb.append(resultZc.getMc()).append("（").append(resultZc.getDm()).append("）");
		resultSb.append("由").append(userService.getWXUser(resultZc.getAppId(), djr).getName());
		resultSb.append("于").append(DateUtils.getLongDate(new Date()));
	...
	public void reallocateZC(Integer zcId, Integer new_zcglId, String new_cfdd, String djr) throws OperException {
		...
		zt.setNewZt(ZCZT.未使用.getValue());
		resultSb.append("资产").append(oldZc.getMc()).append("(").append(oldZc.getDm()).append(")");
		resultSb.append("由").append(userService.getWXUser(oldZc.getAppId(), djr).getName());
		resultSb.append("于").append(DateUtils.getLongDate(new Date()));
	...
	public void agreeRevertSQ(Integer appId, String cjr, List<Integer> zcidList) throws OperException {
		for (Integer zcid : zcidList) {
			...
			zt.setNewZt(ZCZT.已登记.getValue());
			StringBuilder resultSb = new StringBuilder();
			resultSb.append(userService.getWXUser(appId, cjr).getName());
