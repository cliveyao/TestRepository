...
public class StringUtils {
	 ...
	 * @throws Exception
	 */
	public static Map<String, String> getPramsByUrl(String urlString) throws Exception {
		...
		if (!isEmpty(query)) {
			...
			for (String querySplit : queryArray) {
				propArray = querySplit.split("[=]");
				if (propArray.length > 1) {
					mapRequest.put(propArray[0], propArray[1]);
				} else {
					if (!isEmpty(propArray[0])) {
						mapRequest.put(propArray[0], null);
					}
				}
			}
		}
