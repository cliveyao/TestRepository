  ...
  <div id="registYHPDialog">
   <form id="registYhpform" method="post" enctype="multipart/form-data">
		<table align="center" cellspacing="5px" style="margin-top:20px">
		  <tr><td style="text-align: right;">易耗品类型：</td><td><input id="registYHPUI_yhplx" type="text"/><span style="color:#FF0000;margin-left: 3px;">*</span></td></tr>
...
<script type="text/javascript">
	...
	var registYHP = {
			...
			queryYhplx: function(){
			dialogObj=$("<div id='queryYhpLxDialog'></div>");
			dialogObj.appendTo("body");
			cyhplx=null;
			$("#queryYhpLxDialog").dialog({
				title: "查找易耗品类型",
		...
		$('#registYhpform').form('submit', {    
		    ...
		    onSubmit: function(param){    
				...
				if (!isValid){
					...
					if(isValid && (isNaN(limit) || limit+0<0) ){
						$.messager.progress('close');	// 如果表单是无效的则隐藏进度条
						isValid=false;
						$.messager.alert("提示","持有下限不是大于等于0的正整数！",'info');
					}
					if(isValid && num-limit<0){
						$.messager.progress('close');	// 如果表单是无效的则隐藏进度条
						isValid=false;
