...
<html>
  ...
  <body>
	<div id="registYHPListDialog">
		<div id="YHPListToolbar" style="width: 100%;height:30px;padding-top: 10px;">
			...
			<div style="float:right;">
				...
				<div style="float:left;line-height: 25px;">易耗品部门：</div><div style="float:left;padding-right: 10px;"><input style="width:100px" id="QY_YhpDept" name="yhpDept"/></div>
	...
	var registQYYHPList = {
		...
		query: function () {
					var lxId=$("#QY_YhpLx").attr("lx_id");
					var deptno=$("#QY_YhpDept").attr("deptno");
		...
		queryYhplx: function(dialogId,url,title,param,searchboxId){
       	...
       	queryDeptment: function(dialogId,url,title,param,searchboxId){
       		dialogObj = $('<div id="' + dialogId + '"></div>');
				dialogObj.appendTo("body");
				wxdept=null;
				$("#" + dialogId).dialog({
	...
	$("#QY_YhpLx").searchbox({
		...
		searcher: function(value,name){
			registQYYHPList.queryYhplx("queryQYYHPLxDialog","/console/catalog/yhplxgl/queryYhplxUI.do","易耗品类型查找",{isEdit: false},"QY_YhpLx");
		}
	});
	
	$("#QY_YhpDept").searchbox({
		editable: false,
		searcher: function(value,name){
			registQYYHPList.queryDeptment("queryQYYHPDeptDialog", "/console/wxgl/wxdept/queryWXDeptUI.do", "易耗品部门查找", {isEdit: false}, "QY_YhpDept");
		}
	});
