...
<script>
  import api from '@/api'
  import { Cell, Group, Divider, Previewer, TransferDom, Card, Tabbar, TabbarItem } from 'vux'
  ...
  export default {
    ...
    beforeRouteEnter (to, from, next) {
      next(vm => {
        // hidebottomtab
        vm.$store.dispatch('setNavHeadTitle', '资产详情')
        if (from.name === 'Rwxq') {
          vm.$store.dispatch('showNavHeadBack', true)
        } else {
          vm.$store.dispatch('showNavHeadBack', false)
        }
      })
    }
  }
